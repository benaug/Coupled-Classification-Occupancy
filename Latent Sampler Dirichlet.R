                ...
                for(l in 1:y2D[j,k]){
                  if(IDknown3D[j,k,l]){
                    logprobk[k] <- logprobk[k]+logmixprobs[IDtrue3D[j,k,l]]+ #add ecological prior categorical likelihood
                      ddirch(G.obs4D[j,k,l,1:n.species],G.alpha[IDtrue3D[j,k,l],1:n.species],log=TRUE)  #add feature score likelihood
                  }else{
                    logprob.tmp <- rep(0,n.species) #store feature score logprob over all possible species
                    for(i in 1:n.species){
                      logprob.tmp[i] <- ddirch(G.obs4D[j,k,l,1:n.species],G.alpha[i,1:n.species],log=TRUE)
                    }
