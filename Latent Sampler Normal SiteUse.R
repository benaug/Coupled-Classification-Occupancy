                      ...
                      for(l in 1:y2D[j,k]){
                        if(IDknown3D[j,k,l]){
                          logprobww[ww] <- logprobww[ww]+logmixprobs[IDtrue3D[j,k,l]]+ #add ecological prior categorical likelihood
                            dnorm(G.obs3D[j,k,l],G.mu[IDtrue3D[j,k,l]],G.sigma[IDtrue3D[j,k,l]],log=TRUE)  #add feature score likelihood
                        }else{
                          logprob.tmp <- rep(0,n.species) #store feature score logprob over all possible species
                          for(i in 1:n.species){
                            logprob.tmp[i] <- dnorm(G.obs3D[j,k,l],G.mu[i],G.sigma[i],log=TRUE)
                          }
