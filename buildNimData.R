buildNimData <- function(data=data){
  ...
  if(is.matrix(data$G.obs)){
    n.samples=nrow(G.obs)
  }else{
    n.samples=length(G.obs) 
  }
  J=dim(y2D)[1]
  K=dim(y2D)[2]
  ...
  maxdet=max(y2D)
  IDtrue3D=IDknown3D=G.obs3D=array(0,dim=c(J,K,maxdet))
  for(j in 1:J){
    for(k in 1:K){
      ...
      G.obs3D[j,k,1:length(idx)]=G.obs[idx]
    }
  }
  ...
  return(list(zpossible=zpossible,n.community=n.community,
              G.obs3D=G.obs3D,IDknown3D=IDknown3D,IDtrue3D=IDtrue3D,
