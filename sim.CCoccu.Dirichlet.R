sim.CCoccu.Dirichlet <-
  function(n.species=NA,psi.occ=NA,lambda=NA,lambda.sd=NA,K=NA,K2D=NA,J=NA,n.cov=NA,G.alpha=NA,
           pObs=1,pKnown=0){
    if(nrow(G.alpha)!=n.species)stop("G.alpha must be n.species x n.species matrix")
    if(ncol(G.alpha)!=n.species)stop("G.alpha must be n.species x n.species matrix")
    if(!missing(K2D)){
    ...
    G.obs=array(NA,dim=c(n.samples,n.species))
    for(l in 1:n.samples){
      G.obs[l,]=nimble::rdirch(1,alpha=G.alpha[ID[l],])
    }
    ...
    parm.vals <- list(psi.occ=psi.occ,lambda=lambda,lambda.sd=lambda.sd,G.alpha=G.alpha,pObs=pObs,pKnown=pKnown)
